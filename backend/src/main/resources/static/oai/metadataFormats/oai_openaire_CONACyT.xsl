...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <oaire:resource xmlns:vc="http://www.w3.org/2007/XMLSchema-versioning"
            ...
            <datacite:rightsList>
                ...
                <datacite:rights>
                    ...
                    <xsl:text>Creative Commons Attribution 4.0 International</xsl:text>
                </datacite:rights>
            </datacite:rightsList>
            <!-- datacite:subject -->
            <xsl:apply-templates
            ...
            <!-- TODO: datacite:geoLocations -->
            <xsl:apply-templates
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='geoLocations']" mode="datacite" />
        </oaire:resource>
    </xsl:template>
   ...
   <!-- datacite.title -->
    ...
    <!-- This template creates the sub-element <datacite:nameIdentifier> of type RNCTIMX from a Person built entity -->
    <xsl:template match="doc:field[starts-with(@name,'person.identifier.rnctimx')]" mode="entity_author">
        <datacite:nameIdentifier nameIdentifierScheme="RNCTIMX" schemeURI="http://repositorionacionalcti.mx/">
            <xsl:value-of select="./text()"/>
        </datacite:nameIdentifier>
    </xsl:template>
   ...
   <!-- datacite:subject -->
    <xsl:template match="doc:element[@name='dc']/doc:element[@name='subject']/doc:element" mode="datacite">
        <xsl:for-each select="./doc:field[@name='value']">
            <datacite:subject>
                <xsl:attribute name="subjectScheme">CTI</xsl:attribute>
                <xsl:value-of select="./text()"/>
            </datacite:subject>
        </xsl:for-each>
    </xsl:template>
   ...
   <!-- to retrieve a string in lowercase -->
    ...
    <xsl:template name="isURN">
        ...
        <xsl:choose>
            <xsl:when test="$lc_field[starts-with(text(),'oai:')]">
                <xsl:value-of select="true()"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="false()"/>
    ...
    <xsl:template name="resolveFieldType">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$isURN = 'true' and $isURL = 'false' and $isHandle = 'false' and $isDOI = 'false'">
                <xsl:text>URN</xsl:text>
            </xsl:when>
            <xsl:otherwise>
