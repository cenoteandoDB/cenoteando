...
public class CenoteService {
  ...
  public Page<Cenote> getCenotes(Pageable page) {
    ...
    switch (user.getRole()) {
      ...
      case CENOTERO_BASIC:
        if (user.getCenoteWhiteList() == null) user.setCenoteWhiteList(new ArrayList<>());
  ...
  public Cenote createCenote(Cenote cenote) throws Exception {
    if (!cenote.validate()) throw new Exception("Validation failed for Cenote creation.");
  ...
  public Cenote updateCenote(String id, Cenote cenote) throws Exception {
    if (!cenote.validate()) throw new Exception("Validation failed for Cenote update.");
