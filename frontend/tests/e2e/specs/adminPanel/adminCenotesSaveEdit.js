describe('Admin Panel Access', () => {
	...
	it('Select cenote type', () => {
		cy.clickDataCy('cenote-type').get('.v-list-item__title').contains('CENOTE').click({ force: true });
		cy.clickDataCy('cenote-type').get('.v-list-item__title').contains('DRY_CAVE').click({ force: true });
	});
	...
	it('Coordinates', () => {
		cy.clickDataCy('coordinates').type('8970');
	});


    it('Choose Cenote Issues', () => {
		cy.clickDataCy('cenote-issues').get('.v-list-item__title').contains('GEOTAG_NOT_VERIFIED').click({ force: true });
	});

    it('Just exiting cenote issues', () => {
